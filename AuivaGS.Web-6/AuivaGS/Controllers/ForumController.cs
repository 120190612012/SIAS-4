...
namespace AuivaGS.Controllers
{
    ...
    public class ForumController : ApiBaseController
    {
        ...
        [Route("CreateMassage")]
        [HttpPost]
        public async Task<IActionResult> CreatMassege(CreateMassageRespone createMassage)
        {
            var m = await _storedP.CreateMassegeAsync(LoggedInUser.Id, createMassage.Receiver_ID , createMassage.Massage);

            return Ok(m);
        }
        [Route("GetChatForChatID")]
        [HttpPost]
        public async Task<IActionResult> GetChatForChatID(GetChatForChatIDResponen getChatForChatID)
        {
            var m = await _storedP.GetChatForChatIDAsync(getChatForChatID.ChatID);

            return Ok(m);
        }
        [Route("GetChatForUserID")]
        [HttpGet]
        public async Task<IActionResult> GetChatForUserID()
        {
            var m = await _storedP.GetChatForUserIDAsync(LoggedInUser.Id);
